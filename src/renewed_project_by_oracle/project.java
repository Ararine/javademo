...
public class project {
	...
	public static void main(String[] args) {
		...
		while (number != 5) {
			System.out.printf("1.상품등록 2.상품조회 3.상품수정 4.상품제거 5.종료\n");
		...
		if (aList.size() == 0)
			...
			for (ProductDTO dto : aList) {
				System.out.printf("번호 : %-10d \t품목 : %s \t상품명 : %-10s \t가격 : %d원 \t유통기한 : %s년 %s월 %s일까지\n",
						dto.getNum(), dto.getCategory(), dto.getName(), dto.getPrice(),
						dto.getExpireDate().substring(0, 4), dto.getExpireDate().substring(4, 6),
	...
	public static void updateProduct(ProductController pController, Scanner sc) {
		HashMap<String, Object> hmap = new HashMap<String, Object>();
		String searchWord = sc.nextLine();
		System.out.println("수정할 상품을 입력하세요.");
		searchWord = sc.nextLine();
		hmap.put("name", searchWord);
		List<ProductDTO> aList = pController.checkProcess();
		go: for (int i = 0;; i++) {
			for (int j = 0; j < aList.size(); j++) {
				if (aList.get(j).getName().equals(searchWord)) {
					System.out.println("가격을 변경하시려면 1, 유통기한을 변경하시려면 2를 눌러주세요");
					String slt = sc.next();
					if(slt.equals("1")) {
						re:
						for (int l = 0;; l++) {
							...
							System.out.println("가격을 입력하세요.");
							editPrice = sc.next();
							for (int m = 0; m < editPrice.length(); m++) {
								if (!(editPrice.charAt(m) >= '0' && editPrice.charAt(m) <= '9')) {
									System.out.println("잘못입력하셨습니다.");
									continue re;
								}
							}
							hmap.put("price", Integer.parseInt(editPrice));
							pController.updatePriceProcess(hmap);
							System.out.printf("%s 가격이 %s원으로 변경되었습니다.\n", searchWord, editPrice);
							break;
						}
						...
						while(psh) {
							...
							editExpireDate = sc.next();
							if(editExpireDate.length() != 8){
								System.out.println("잘못입력하셨습니다.");
							}else break;
						}
						hmap.put("expireDate", editExpireDate);
						pController.updateExpireDateProcess(hmap);						
						System.out.printf("%s 상품 유통기한이 %s년 %s월 %s일까지로 변경되었습니다.\n", searchWord,
								editExpireDate.substring(0, 4), editExpireDate.substring(4, 6),
								editExpireDate.substring(6, 8));
					} else {
