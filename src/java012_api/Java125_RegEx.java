...
public class Java125_RegEx {
	...
	public static void main(String[] args) {
		...
		System.out.println(sn.matches(".*[^0-9].*")); //처음에 ^ : not
		System.out.println(sn.matches(".*[\\D].*")); // \\D : !0~9
		...
		System.out.println(st.matches("[ab].{5}")); // {5} : 임의의 문자 5개
		
		//st변수에 저장된 값이 a이거나 b로 시작하고 임의문자가 1~5개이며 cc로 끝나면 true 아니면 false를 리턴
		st = "aetwefcc";
...
//		input1 = "12";
		System.out.println(input1.matches("\\d")); // \\d는 숫자 하나
        ...
        */
		...
		int i = 1;
		while(mh.find())
			System.out.printf("%d번째 %d~%d %s\n", i++, mh.start(), mh.end(), mh.group()); //1번째 4~8 BEst 2번째 19~23 best 3번째 33~37 best
		...
		i = 1;
		while(mh.find())
			System.out.printf("%d번째 %d~%d %s\n", i++, mh.start(), mh.end(), mh.group()); //1번째 4~10 BEst10 2번째 21~27 best20 3번째 37~43 best30
