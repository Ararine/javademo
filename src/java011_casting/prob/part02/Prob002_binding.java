...
public class Prob002_binding {
	public static void main(String[] args) {
		...
		int[] specialBonus = new int[em.length];
		
		for(int i=0; i<em.length; i++) {
			if(i%2 == 0) specialBonus[i] = (int)(em[i].salary * 0.13);
			else specialBonus[i] = (int)(em[i].salary * 0.15);
		}
		

		for(int i=0; i<em.length; i++) {
			System.out.print(em[i].toString() + "  ");
			System.out.println(specialBonus[i]);
		}
		//////////////////////////////////////
